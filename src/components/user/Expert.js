  ...
  return (
    <section id="expert" className="expert-system">
      <div className="container">
        <div className="expert-system__intro">
          <h2 className="expert-system__title">
            🧭 Tvoj Expertný Poradca
          </h2>
          <p className="expert-system__subtitle">
            Postupuj krok za krokom a nájdi odpovede, ktoré potrebuješ
          </p>
        </div>

        <div className="expert-system__card">
          {/* Header s progress indikátorom */}
          <div className="expert-system__cardHeader">
            <div className="expert-system__stepLabel">
              {history.length > 0 && `Krok ${history.length + 1}`}
              {history.length === 0 && "Úvod"}
            </div>
            <h3 className="expert-system__nodeTitle">
              {node.title}
            </h3>
          </div>

          <div className="expert-system__cardBody">
            <p className="expert-system__questionText">
              {node.text}
            </p>

            {node.type === "question" && (
              <div className="expert-system__options">
            ...
            {node.type === "result" && (
              <div className="expert-system__result">
                <h4 className="expert-system__resultTitle">
                  💡 {node.title}
                </h4>
                <ul className="expert-system__recs">
                  {node.recommendations.map((r, i) => (
                    <li key={i} className="expert-system__rec">
                      <span className="expert-system__recCheck">✓</span>
                      {r}
                    </li>
                  ))}
                </ul>
                <div className="expert-system__safety">
                  <p className="expert-system__safetyText">
          ...
          {/* Footer s tlačidlami */}
          <div className="expert-system__footer">
            <button 
              onClick={goBack}
              disabled={history.length === 0}
              className="expert-system__btnBack"
            >
              ← Späť
            </button>
            <button 
              onClick={reset}
              className="expert-system__btnReset"
            >
              🔄 Začať odznova
            </button>
          </div>
